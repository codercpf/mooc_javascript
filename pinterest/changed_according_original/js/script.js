...
// 第一步：页面加载时，对所有图片进行静态布局	
	waterfall('main','pin');	//对main中的所有图片所在的div(class为pin)做操作
...
// 第二部：监听滚动条，加载更多图片
	...
	window.onscroll=function(){
		...
		// checkScrollSlide();			//这里一定要写成函数形式即()形式，否则只要滚动就会加载（不解）
		if (checkScrollSlide()) {
			var oParent = document.getElementById('main');
			// 加载后台的更多数据, 数据块渲染到页面尾部
			for(var i=0; i<dataInt.data.length; i++){
				var oBoxNew = document.createElement('div');
				oBoxNew.className = 'pin';

				//将新产生的div添加到父元素main的后面
				oParent.appendChild(oBoxNew);			//放数据块的div class=box创建好了

				var oPicNew = document.createElement('div');	//创建放图片的div class=box，并追加到box尾部
				oPicNew.className = 'box';
...
function getByClass(parent, clsName){
	...
	var	oElements = parent.getElementsByTagName('*');	//指定名称是，获取特定名称的元素；不指定名称获取所有元素
...
function checkScrollSlide(){
	...
	/*
		新解：创建【触发添加块函数waterfall()】的高度
		最后一个块框的距离网页顶部+自身高的一半（实现未滚动到底就开始加载）
	*/
	var lastBoxH = oBoxs[oBoxs.length - 1].offsetTop + Math.floor(oBoxs[oBoxs.length-1].offsetHeight / 2);

	// 参考距离为，页面浏览器的高度+向上滚动过的距离
	//混杂模式下，通过body获取滚走的距离；标准模式下，通过documentElement获取滚走的距离
	var scrollTop = document.body.scrollTop || document.documentElement.scrollTop;		//滚走高度，解决兼容性
